...
export function healthPanel(data) {
  ...
  const rows = models.length
    ...
    : [];
  return { flags: V2, components: [panel({ title: `LOOP · ${KINDS.health}`, body: rows.length ? `${body}\n\n${rows.join('\n')}` : body, footer: `Snapshot at ${data.observedAt}`, buttons: controls('health') })] };
}
...
export function budgetPanel(data) {
  ...
  return { flags: V2, components: [panel({ title: `LOOP · ${KINDS.budget}`, body: rows.length ? `${body}\n\n${rows.join('\n')}` : body, footer: `Period starts at ${data.periodStart}`, buttons: controls('budget') })] };
}
...
export function approvalsPanel(rows) {
  ...
  return { flags: V2, components: [panel({ title: `LOOP · ${KINDS.approvals}`, body, footer: 'Pending proposals awaiting owner decision', buttons: controls('approvals') })] };
}
...
export function policiesPanel(rows) {
  ...
  return { flags: V2, components: [panel({ title: `LOOP · ${KINDS.policies}`, body, footer: 'Set per-domain autonomy via the policies store', buttons: controls('policies') })] };
}
