...
export class ContextAssembler {
  ...
  async assemble({messageId,guildId,channelId,threadId,userId,maxTokens=this.maxTokens}) {
    ...
    for(const m of exact)if(m.authorId&&aliasMap.has(m.authorId))m.author=aliasMap.get(m.authorId);
    ...
    const context={identity:'Azure',observedAt:new Date().toISOString(),exactReferenceChain:exact,recentMessages:recent,activeTasks:tasks,userMemories:memories,guildFacts,semanticMemories:semantic,userAlias:aliasMap.get(userId)??null};
