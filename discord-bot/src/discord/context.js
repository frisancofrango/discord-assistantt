...
export class ContextAssembler {
  ...
  async assemble({messageId,guildId,channelId,threadId,userId,maxTokens=this.maxTokens,lean=false}) {
    const [exact,tasks,memories,guildFacts,stateDigests]=await Promise.all([
      ...
      this.db.query(`SELECT kind,content,metadata,updated_at FROM semantic_memories
        WHERE kind='state'
          AND ($1::text IS NULL OR metadata->>'discordGuildId'=$1)
          AND ($2::text IS NULL OR metadata->>'discordUserId'=$2)
          AND ($3::text IS NULL OR metadata->>'discordChannelId'=$3)
        ORDER BY updated_at DESC LIMIT 3`,[guildId??null,userId??null,channelId??null]).then((r)=>r.rows),
    ]);
  ...
  #budget(c,max){ const removable=['semanticMemories','guildFacts','userMemories','activeTasks','recentMessages','channelState']; for(const key of removable){while(estimate(c)>max&&c[key].length)c[key].shift();} if(estimate(c)>max) for(const m of c.exactReferenceChain)m.content=trimText(m.content,1000); }
}
