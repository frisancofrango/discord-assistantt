...
export class EngagementPolicy {
  ...
  decide(input) {
    ...
    let score=0; if(taskWords.test(input.content??''))score+=.35; if(input.question)score+=.2; if(input.azureRelevant)score+=.3; if(input.recentAzureContext)score+=.12; if(complaint.test(input.content??''))score+=.4; if(input.lowSignal)score-=.35;
