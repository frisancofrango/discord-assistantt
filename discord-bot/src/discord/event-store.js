...
import { normalizeGatewayEvent, normalizeMessage, stableHash } from './normalize.js';
...
export class DiscordEventStore {
  ...
  async ingest(eventType, entity, extra = {}, client = this.db) {
    ...
    const inserted = await client.query(`INSERT INTO discord_events(gateway_key,event_type,guild_id,channel_id,thread_id,user_id,resource_id,occurred_at,correlation_id,payload)
      ...
      [event.gatewayKey,event.eventType,event.guildId,event.channelId,event.threadId,event.userId,event.resourceId,event.occurredAt,correlation,JSON.stringify(event.payload)]);
  ...
  async #persistMessage(event,client) {
    ...
    if(!message) message=(await client.query(`INSERT INTO messages(conversation_id,discord_id,author_id,content,metadata,referenced_message_id,last_discord_edited_at,deleted_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,[conversation.id,p.id,user?.id??null,p.content,JSON.stringify(metadata),p.reference?.messageId??null,p.editedAt,p.deleted?event.occurredAt:null])).rows[0];
    ...
    if(!latest || latest.metadata?.hash!==metadata.hash || p.deleted) {
      ...
      await client.query(`INSERT INTO message_revisions(message_id,revision,content,metadata) VALUES($1,$2,$3,$4)`,[message.id,revision,p.content,JSON.stringify({...metadata,eventType:event.eventType,deleted:p.deleted,observedAt:event.occurredAt})]);
    }
    await client.query(`UPDATE messages SET content=$2,metadata=$3,referenced_message_id=$4,last_discord_edited_at=$5,deleted_at=CASE WHEN $6 THEN $7 ELSE deleted_at END,updated_at=now() WHERE id=$1`,[message.id,p.content,JSON.stringify(metadata),p.reference?.messageId??null,p.editedAt,p.deleted,event.occurredAt]);
  }
}
